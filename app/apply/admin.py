...
def set_interview_time(
    ...
    # sorted_time_counts = sorted(time_count.items(), key=lambda item: item[1])
    sorted_time_counts = (
    ...
    # 정렬된 InterviewTime 객체들의 수요도(applicant_count)와 시간(time) 출력
    # for interview_time in sorted_time_counts:
    #     print(
    #         f"InterviewTime ID: {interview_time.id}, Applicant Count: {interview_time.applicant_count}, Time: {interview_time.time}"
    #     )
    ...
    # 면접 시간 수요도(time_count) 출력
    # print("면접 시간별 수요도:")
    # for interview_time_id, count in time_count.items():
    #     print(f"InterviewTime ID: {interview_time_id}, Demand Count: {count}")

    # # 지원자가 할당된 횟수(fixed_applicant_count) 출력
    # print("면접 시간별 할당된 지원자 수:")
    # for interview_time_id, count in fixed_applicant_count.items():
    #     print(
    #         f"InterviewTime ID: {interview_time_id}, Assigned Applicant Count: {count}"
    #     )

    # if choices.time in times:
    #     time_num = 0
    #     s = Resume.objects.annotate()
    #     for (
    #         _resume
    #     ) in s:  # resume에 대해 call하는 부분 문제 어떤방식으로 call해야하나?
    #         if choices.time == _resume.fixed_interview_time:
    #             time_num = time_num + 1
    #     if time_num == 1:
    #         plus20 = choices.time + datetime.timedelta(minutes=20)
    #         a = Resume.objects.annotate()
    #         for Res in a:
    #             if plus20 == Res.fixed_interview_time:
    #                 time_num = time_num + 1
    #         if time_num == 2:  # 2개 있을 때
    #             applicant.fixed_interview_time = plus20 + datetime.timedelta(
    #                 minutes=20
    #             )
    #             applicant.save()
    #             choices.is_fixed = True
    #             choices.save()
    #             times.remove(choices.time)
    #             break
    ...
    #     else:  # 0개일때
    #         applicant.fixed_interview_time = choices.time
    #         applicant.save()
    #         break
